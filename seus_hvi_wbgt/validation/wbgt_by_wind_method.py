...
def main( *args, methods = None, nCol = 7, **kwargs):
  ...
  bins        = [0, 0.1, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0, 6.0, 8.0, 10.0, 15.0]
  nBins       = len( bins )
  ...
  for color, (location, df) in enumerate( data.groupby('location') ):           # Iterate over all stations within the dataframe
    ...
    for midx, method in enumerate( methods ):
      ...
      for vidx, (var, xx) in enumerate( zip( variables, [Tglobe, Tpsy, Tnwb, Twbgt] ) ):
        ...
        colors = numpy.digitize( speed.m, bins ) / nBins 
  ...
  clines = [ Line2D([0], [0], color=cmap(c/nBins), alpha=ALPHA, lw=4) for c in range(nBins)]       # Append line for the station in the legend

  fig.legend( flip(clines, nCol), flip(bins, nCol),
         ncol     = nCol, 
         loc      = [0.02, 0.02],
         fontsize = 'x-small'
  ) 
  ...
  fig.savefig( '/Users/kwodzicki/Documents/NCICS/NOAA_Dello_113020_SE-HVI-WBGT/Figures/WBGT_by_winds_method.png',
    dpi = 300 ) 
